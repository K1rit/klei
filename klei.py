...
def open_play_game():
    ...
    button_exit = Button(window_play_game, text="Сбежать", font=font_button, command = window_play_game.destroy, width=10, pady=3).place(relx=0.85, rely=0.9)
...
def open_authors():
    ...
    button_exit = Button(window_authors, text="ОК", font=font_button, command=window_authors.destroy, width=10).place(relx=0.5, rely=0.90, anchor=CENTER)
...
button_exit = Button(window, text="Выход", font=font_button, command = window.quit, width=30, pady=3).place(relx=0.5, rely=0.50, anchor=CENTER)
